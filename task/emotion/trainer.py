...
def main():
    ...
    # Handle dynamic target names based on what's actually in the test set
    unique_labels = sorted(test_df['label'].unique())
    filtered_names = [target_names[i] for i in unique_labels]
    print(classification_report(test_df['label'], dl_preds, labels=unique_labels, target_names=filtered_names, zero_division=0))
